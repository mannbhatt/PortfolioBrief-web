...
${para}`, { waitUntil: 'networkidle2' });

            const mainSec = await page.$('.mainSec');
            const myListView = await mainSec.$('#mylistView');
//console.log("1.5",myListView)
            const listToStoryElements = await myListView.$$('#listview');
		//console.log("2",listToStoryElements)
            const extractedData = [];
...
*/
	console.log("3")
         for (const element of listToStoryElements) {
                ...
                const images = await element.$$('.thumbnail img');
                for (let i = 0; i < headlineSections.length; i++) {
                    ...
                    if (image) {
 imageUrl = await page.evaluate((img) => img.getAttribute('src'), image);                        
                    }
            		console.log()
                    extractedData.push({
                        innerText: innerText,
