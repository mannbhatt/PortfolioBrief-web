...
async function structureEmail(data) {
    let newsContent = "";
...
//<p style="margin-top: 16px;">We are working hard so you don't get message like this again :)</p>
    if (data.news.length === 0) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if(News.length == 5 || !!News[i]){break;}else{
            let sourceContent = null;
            
            if (News[i].href && News[i].href.includes("https://www.livemint.com/")) {
               
                sourceContent =" LiveMint";
            }
            else if (News[i].href && News[i].href.includes("https://www.moneycontrol.com/")) {
                sourceContent =" MoneyControl";
            }
        ...
        <div style="margin: 0; padding: 0; font-family: Arial, sans-serif; display: flex; gap: 4px; padding-top: 18px;">
        <div class="news" style="margin: 0; padding: 0; font-family: Arial, sans-serif; text-align: left; width: auto;">
<a href="${News[i].href}" target="_blank">${News[i].innerText}</a><p></p>
<div>
<span style="margin: 0; padding: 0; font-family: Arial, sans-serif; font-size: 14px;">${formatTime(News[i].newsTime)}</span><span class="tag" style="margin: 0; padding: 0; font-family: Arial, sans-serif; font-size: 14px;"></span>
• <span class="tag" style="margin: 0; padding: 0; font-family: Arial, sans-serif; font-size: 14px;">${sourceContent}</span>
</div>
</div>
<img src="${News[i].imageUrl}" alt="News Image" style="margin: 0; padding: 0; font-family: Arial, sans-serif; width: 80px; height: 80px;" width="80" height="80"></div>
...
async function sendData() {
    ...
    try {
        for (let i = 0; i < stocklist.length; i++) {
            ...
            if (emailContent.to === "manb35624@gmail.com"){ //stocklist[i].userEmail
